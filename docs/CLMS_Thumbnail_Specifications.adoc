...
|Color Depth
|24-bit RGB with 8-bit alpha channel (RGBA)

|Compression
|PNG adaptive filtering with deflate compression

|Interlacing
|Non-interlaced (sequential encoding)
...
|Color Ramp Format
|Available as LUT (Look-Up Table), QGIS .qml style files, or color definition files
|===
...
Regional thumbnails::
`{PRODUCT}_{VERSION}_{REGION}_{DATE}.png`

Global thumbnails::
`{PRODUCT}_{VERSION}_globe_{DATE}.png`
...
|`{PRODUCT}`
|Product identifier including spatial resolution
|`CLMS_DMP300mV2`, `CLMS_NDVI300mV3`

|`{VERSION}`
|Product version (if applicable, may be included in PRODUCT)
|`V2`, `V3`

|`{REGION}`
|Geographic region name (lowercase, no spaces)
|`brazil`, `europe`, `africa`, `asia`

|`{DATE}`
|Temporal reference in YYYYMMDD format
|`20250630`
|===
...
----
CLMS_DMP300mV2_Brazil_20250630.png
CLMS_DMP300mV2_globe_20250630.png
CLMS_NDVI300mV3_Europe_20241231.png
CLMS_NDVI300mV3_globe_20241231.png
CLMS_FAPAR300m_Africa_20250315.png
CLMS_LAI300m_globe_20250101.png
----
...
*Document Control*: This specification is maintained by the CLMS Global Operations team. Updates require review and approval by the Technical Coordination Board.
